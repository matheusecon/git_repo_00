...
procuroutrabNosult7d 541-541

procuroutrabNosult23d 542-542

procuroutrabNosult30d 543-543
...
drop if AnoNasc < 1950
drop if AnoNasc > 1999
...
gen nascimento=.
replace nascimento = 1 if AnoNasc >=1960 & AnoNasc <=1965
replace nascimento = 2 if AnoNasc >=1966 & AnoNasc <=1970
replace nascimento = 3 if AnoNasc >=1971 & AnoNasc <=1975
replace nascimento = 4 if AnoNasc >=1976 & AnoNasc <=1980
replace nascimento = 5 if AnoNasc >=1981 & AnoNasc <=1985
replace nascimento = 6 if AnoNasc >=1986 & AnoNasc <=1990
replace nascimento = 7 if AnoNasc >=1991 & AnoNasc <=1995
replace nascimento = 8 if AnoNasc >=1996 & AnoNasc <=1998
...
collapse (mean) ano controle serie ref ordem FRMDPC RMDPC militar funpublico Sexo DiaNasc MesNasc AnoNasc Idade CondFam Cor MoracMae EstCiv LugNasc TempMoraUF TempMoraMun Alfabet Estud AcesNet3m SeTrabDom Pensao SeRendFinanc numtotalfilho TempoEstudo EconAtiva desocupado D3 D4 D5 pesopes [fw=pesopes], by(cohorts)

drop if cohorts ==.
***Analise o editor de dador e verifique que os dados agora se limitam a 18 observações***
***A função "round" tem por objetivo o arredondamento para número inteiro***
gen peso_novo= round(pesopes,1)

gen sexo_novo= round(Sexo,1)

gen cor_novo= round(Cor,1)

order cohorts ano controle serie ref ordem FRMDPC RMDPC militar funpublico Sexo sexo_novo DiaNasc MesNasc AnoNasc Idade CondFam Cor cor_novo MoracMae EstCiv LugNasc TempMoraUF TempMoraMun Alfabet Estud AcesNet3m SeTrabDom Pensao SeRendFinanc numtotalfilho TempoEstudo EconAtiva desocupado D3 D4 D5 pesopes peso_novo
...
gen Deflacionamento =.
replace Deflacionamento =0.6267 if ano ==2015
replace Deflacionamento =0 if Deflacionamento ==.

save coortes_2015, replace
